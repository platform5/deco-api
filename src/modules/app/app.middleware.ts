import { UserModel, AppModel, ObjectId, Query, ApiKey, AppUserItem } from '../../';
import { Request, Response, NextFunction } from 'express';
import moment = require('moment');
let debug = require('debug')('app:middleware:app');
export class AppMiddleware {

  static fetchWithPrivateKey(req: Request, res: Response, next: NextFunction) {
    let privateKey: string | undefined = req.query.apiKey as string;
    if (!privateKey) return next(new Error('Missing apiKey'));
    let query = {
      "privateKeys": {
        $elemMatch: {
          key: privateKey,
    ...
    AppModel.getOneWithQuery(query).then((element) => {
      if (!element) return next(new Error('Invalid API Key'));
      res.locals.app = element;
      next();
    }).catch(next);
  }

  static fetchWithPublicKey(req: Request, res: Response, next: NextFunction) {
    let publicKey: string | undefined = req.query.apiKey as string;
    if (!publicKey) return next(new Error('Missing apiKey'));
    let query = {
      "publicKeys": {
        $elemMatch: {
          key: publicKey,
  ...
  static fetchParamApp(requireUserAccessToParamApp: boolean = true) {
    return (req: Request, res: Response, next: NextFunction) => {
      if (!req.params || !req.params.appId) return next(new Error('Missing appId - did you forget to add a :appId in the route ?'));
      if (!res.locals.app) return next(new Error('Missing app - did you forget to fetchWithPublicKey'));
      let rightInstance = res.locals.app instanceof AppModel;
      if (!rightInstance) return next(new Error('Invalid app, not instance of AppModel'));
      if (requireUserAccessToParamApp) {
        if (!res.locals.user) return next(new Error('Missing user - did you forget to authenticate'));
        rightInstance = res.locals.user instanceof UserModel;
        if (!rightInstance) return next(new Error('Invalid user, not instance of UserModel'));
      }
      try {
        ...
        AppModel.getOneWithQuery(query).then((a) => {
          let app: AppModel = (a as AppModel);
          if (!app) return next(new Error('Invalid appId or access denied'));
          res.locals.paramApp = app;
          let query = (res.locals.query) ? res.locals.query : new Query();
          const rightInstance = query instanceof Query;
          if (!rightInstance) throw new Error('res.locals.query is not a valid Query object');
  ...
  static addAppIdToBody(prop = 'appId') {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (req.body.appId.toString() === res.locals.app?._id.toString()) {
  ...
  static outputKey() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (index.toString() !== req.params.index) return next(new Error('Invalid request'));
      
      let keys = res.locals.element[`${type}Keys`];

      if (!keys || !Array.isArray(keys)) return next(new Error('Invalid Key Property (missing or not array)'));
      if (keys.length < index + 1)  return next(new Error('Invalid Key Property (missing requested index)'));
  ...
  static createKey() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      (res.locals.element as AppModel).update(fields).then((app) => {
  ...
  static editKey() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!res.locals.element) return next(new Error('Missing Element - did you forget to add a getOne middleware?'));
      if (typeof res.locals.element !== 'object') return next(new Error('Invalid Element (not object)'));
      let rightInstance = (res.locals.element instanceof AppModel);
      if (!rightInstance) return next(new Error('Invalid Element (not AppModel)'))
      if (req.params.type !== 'private' && req.params.type !== 'public') return next(new Error('Invalid request'));
      let type = req.params.type;
      ...
      let key = keys[index];
      if (key.key && key.key.substr(-4) !== req.params.last4) return next(new Error('Invalid Key last4 and index'));
      
      if (req.body.name && typeof req.body.name === 'string') key.name = req.body.name;
      if (typeof req.body.active === 'boolean') key.active = req.body.active;
      if (req.body.expires && typeof req.body.expires === 'string') {
        let date = moment(req.body.expires);
        if (date.isValid()) key.expires = date.toDate();
      }
  ...
  static addUser() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      for (let user of app.users) {
        if (user._id.toString() === req.params.userId) return next(new Error('User is already invited in this app'));
      }

      UserModel.getOneWithId(req.params.userId).then((user) => {
  ...
  static editUser() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      let editedFields: Array<string> = [];

      UserModel.getOneWithQuery({appId: res.locals.paramApp._id, _id: userId}).then((u) => {
  ...
  static editParentUser() {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (edited) {
        ...
        if (!found) return next(new Error('User is not in the app'));
        next();
      }
    }
  }
